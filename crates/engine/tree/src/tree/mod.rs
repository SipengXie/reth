...
use reth_primitives_traits::{
    Block, BlockBody, GotExpected, NodePrimitives, RecoveredBlock, SealedBlock, SealedHeader, SignedTransaction,
};
    ...
    V: EngineValidator<T, Block = N::Block>,
{
    ...
    fn insert_block(
        ...
        let span: Span = info_span!(target: "block_profiler", "block", block_num = block_num);
        let _guard = span.enter();
        let result = self.insert_block_inner(block.clone())
    ...
    fn insert_block_inner(
        ...
        let executor_span = info_span!("execute_block");
        let executor_span_guard = executor_span.enter();
        ...
        // Debug output: save receipts to file based on ENABLE_SSA env variable
        // {
        //     let receipts = &output.result.receipts;
        //     let transactions = block.body().transactions();

        //     if !receipts.is_empty() && receipts.len() == transactions.len() {
        //         // Determine output file based on ENABLE_SSA environment variable
        //         let output_file = if env::var("ENABLE_SSA").unwrap_or_default() == "true" {
        //             "receipts_ssa.json"
        //         } else {
        //             "receipts_seq.json"
        //         };

        //         // Serialize receipts to JSON and save to file
        //         if let Ok(json_data) = serde_json::to_string_pretty(receipts) {
        //             if let Err(e) = fs::write(output_file, json_data) {
        //                 warn!(target: "engine::tree", file = output_file, error = ?e, "Failed to write receipts to file");
        //             } else {
        //                 debug!(target: "engine::tree", file = output_file, receipts_count = receipts.len(), "Saved receipts to file");
        //             }
        //         } else {
        //             warn!(target: "engine::tree", "Failed to serialize receipts to JSON");
        //         }

        //         // Save transaction hashes to file (unified filename for both SSA and seq)
        //         let tx_hashes: Vec<B256> = transactions.iter().map(|tx| *tx.tx_hash()).collect();
        //         let tx_hash_file = "transaction_hash.json";

        //         if let Ok(json_data) = serde_json::to_string_pretty(&tx_hashes) {
        //             if let Err(e) = fs::write(tx_hash_file, json_data) {
        //                 warn!(target: "engine::tree", file = tx_hash_file, error = ?e, "Failed to write transaction hashes to file");
        //             } else {
        //                 debug!(target: "engine::tree", file = tx_hash_file, tx_count = tx_hashes.len(), "Saved transaction hashes to file");
        //             }
        //         } else {
        //             warn!(target: "engine::tree", "Failed to serialize transaction hashes to JSON");
        //         }
        //     }
        // }
        ...
        let root_span = info_span!("calculate_state_root");
        let root_span_guard = root_span.enter();
        let mut maybe_state_root = None;

        let header:&dyn BlockHeader = block.header();
        ...
        if run_parallel_state_root {
            ...
            if self.config.use_state_root_task() {
                match handle.state_root() {
                    Ok(StateRootComputeOutcome { state_root: computed_state_root, trie_updates }) => {
                        let elapsed = execution_finish.elapsed();
                        info!(target: "engine::tree", ?computed_state_root, ?elapsed, "State root task finished");
                        // we double check the state root here for good measure
                        if computed_state_root == header.state_root() {
                            maybe_state_root = Some((computed_state_root, trie_updates, elapsed))
                        } else {
                            warn!(
                                target: "engine::tree",
                                ?computed_state_root,
                                block_state_root = ?header.state_root(),
        ...
        let (computed_state_root, trie_output, root_elapsed) = if let Some(maybe_state_root) =
            maybe_state_root
        {
            maybe_state_root
        } else {
        ...
        if computed_state_root != header.state_root() {
            // call post-block hook
            self.on_invalid_block(&parent_block, &block, &output, Some((&trie_output, computed_state_root)));
            return Err(ConsensusError::BodyStateRootDiff(
                GotExpected { got: computed_state_root, expected: header.state_root() }.into(),
            )
            .into())
        }
        ...
        let span = info_span!("__parse_block__",  txs = txs, gas_used = gas_used);
        drop(span.enter());
